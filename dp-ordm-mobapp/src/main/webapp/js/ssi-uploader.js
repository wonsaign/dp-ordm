...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
         ...
         '<button class="ssi-button bg-green submit_left">' + this.language.chooseFiles + '</button>' +
         '</span>').append(element);
        var $uploadBtn = $("#ssi-uploadBtn");
        var $abortBtn = $('<span class="inBtn">' + this.language.abort + ' </span>');
        ...
        $chooseBtn.find('button').click(function () {
            $input.trigger('click');
            $("#ssi-uploadBtn").attr("disabled",false);
            $("#ssi-uploadBtn").removeClass("bg-black");
            $("#ssi-uploadBtn").addClass("bg-green");
        });
...
//----------------------------UPLOADFILES------------------------------------
        $uploadBtn.click(function () {// upload the files
        	if($("#paymentPrice").val() =="" || $("#paymentPrice").val() == null ){
 				$(".message_block2").html("<div>支付金额不能为空!</div>");
 				$(".message_body").fadeIn();
	 	 		$(".message_body").fadeOut(3000);
        	}else{
        		/*if($("#paymentPrice").val() == $("#paymentFee").text()){
        			thisS.uploadFiles();
        		}else{
        			var paymentFee=$("#paymentFee").text();
     				$(".message_block2").html("<div><span style='color:red'>支付金额</span>必须等于<span style='color:red'>应付金额"+paymentFee+"</sapn>!</div>");
     				$(".message_body").fadeIn();
		 	 		$(".message_body").fadeOut(3000);
        		}*/
        		thisS.uploadFiles();
            ...
            var ajaxOptions = $.extend({}, {//store the request to the uploadList variable
                ...
                error: function (request, error) {
                    if (error !== 'abort') {
        				...
        				$(".message_body").show();
        				$(".message_block2").html("<div>请重新添加图片提交</div>");
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                function cb(result, data) {
                    if (result) {//if response type is success
                        ...
                        $(".message_body").show();
                        var accessory_body=$(".accessory_body").html();
        				$(".message_block2").html("<div class='message_top'>提交</div><div class='message_center'>"+accessory_body+"</div>");
        				$(".paymentPrice").text($("#paymentPrice").val());
        				$(".description").text($("#description").val());
        				var $clearBtn = $('.cancel_all');
    					$clearBtn.click(function () { //choose files completed and pending files
    						$(".message_body").hide();
    						thisS.clear();
    						$("#ssi-uploadBtn").attr("disabled",true);
    			            $("#ssi-uploadBtn").removeClass("bg-green");
    			            $("#ssi-uploadBtn").addClass("bg-black");
    			        });
        				$(".sure_all").click(function(){
        					$(".message_body").hide();
        				})
                    } else {
                        uploadBar.addClass('ssi-canceledProgressBar');
