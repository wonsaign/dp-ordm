...
$(document).ready(function(){
					...
					$.each(data,function(i,product){
						...
						if(product.typeId=="11388"||product.typeId=="11389"){
							thead+="<form class='addcart' action='' method='post'>"
								...
								+product.imageURL+"'></a>"
								if(product.prePro==0){
									if(product.inv[stockid]<=300 && product.inv[stockid]>0 ){
										thead+="<div class='inventory_inv' title='"+product.inv[stockid]+"'>库存:"+ product.inv[stockid] +"</div>"
									}
									if(product.inv[stockid]<=0 || product.inv[stockid]=="undefined" || product.inv[stockid]=="" || product.inv[stockid]==null){
										thead+="<div class='inventory' title='"+product.inv[stockid]+"'>暂时无货</div>"
									}
								}else if(product.prePro==3){
									thead+="<div class='inventory'>打欠结束</div>"
								}
							thead+="<input class='actFlag' type='hidden' value="+product.actFlag+">"
								...
								if(product.prePro==0){
									if(product.inv[stockid]<=0 || product.inv[stockid]=="undefined" || product.inv[stockid]=="" || product.inv[stockid]==null){
										thead+="<input class='bg-red add_button' type='submit' value='购物车' disabled='disabled' style='background: #999;'>"
									}else{
										thead+="<input class='bg-red add_button' type='submit' value='购物车'>"
									}
								}else if(product.prePro==1){
									thead+="<input class='bg-red add_button' type='submit' value='购物车'>"
								}else if(product.prePro==2){
									thead+="<input class='bg-red add_button' type='submit' value='打欠中'  style='background: #6dbb44; border-color: #6dbb44;'>"
								}else if(product.prePro==3){
									thead+="<input class='bg-red add_button' type='submit' value='还欠中'  style='background: #999;' disabled='disabled'>"
								}
							thead+="<div class='clear'></div></div>"
								+"</div></div></div>"
								+"</form>";
							item.append(thead);
						}
	...
	$(".text_search input").keyup(function(){
        if(event.keyCode == 13){
	...
	$(window).scroll(function(){
		...
		var tag = $(".show_item").find(".item").length;
...
/*
			...
			setTimeout(scrolled=false, 10000);
		}
					...
					success:function(pGroups){
						...
						$.each(pGroups,function(i,pgroup){
							thead="<form class='addcart' action='' method='post'>"
								+"<div class='item'>"
								+"<div class='item_number'>"
								+"<div class='mobile_img'><a target='_blank' href='../product/activity_"+pgroup.actId+".do' class='product_a'><img class='item_img' src='"
				...
				success:function(pGroups){
					...
					$.each(pGroups,function(i,act){
						thead="<form class='addcart' action='' method='post'>"
							+"<div class='item'>"
							+"<div class='item_number'>"
							+"<div class='mobile_img'><a target='_blank' href='../product/activity_"+act.actId+".do' class='product_a'><img class='item_img' src='"
