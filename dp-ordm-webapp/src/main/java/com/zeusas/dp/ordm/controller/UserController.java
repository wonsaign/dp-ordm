...
public class UserController extends OrdmBasicController {
	...
	public String getUserInfo(ModelMap mp, HttpServletRequest request) throws IOException {
		...
		try {
			...
			for (Counter counter : (List<Counter>)super.getAllCounters()) {
	...
	public String updateUserInfo(UserDetail userBean, @RequestParam(value = "oldPwd", required = false) String oldPwd,
		...
		String username = request.getRemoteUser();
		try {
			if (newPwd == null || newPwd.length() < 5 || username == null) {
				logger.warn("username of session is timeout,or password is not null ,or it`s length is less than 6");
				return null;
			}
			AuthUser auther = authCenterManager.getAuthUser(username);
	...
	public String setIndex(ModelMap mp, HttpServletRequest request) throws IOException {
		...
		try {
			...
			logger.warn("method : setIndex()",e.getMessage());
		} catch (Exception e) {
	...
	public String findUsers(ModelMap mp, @RequestParam(value = "searchkey", required = false) String searchkey,
		...
		try {
			...
			logger.warn("method : findUsers()",e.getMessage());	
		} catch (Exception e) {
	...
	public String useradd(HttpServletRequest request, Model model) throws IOException {
		...
		try {
			...
			if(StringUtil.isEmpty(username)) {
				throw new  ActionException("请求用户为空");
			}
	...
	public String userupdate(@RequestParam(value = "loginName") String loginName, HttpServletRequest request,
		...
		try {
			...
			allCounters.addAll((List<Counter>)super.getAllCounters());
			// 当前登录获取自己柜台
			...
			// 老板标志 禁止修改柜台角色
			if (myUserCustomer.getUserId().equals(myUserCustomer.getCustomerUserId())) {
				idboss = true;
			}
	...
	public String changeStatus(ModelMap mp,
		...
		try {
			if(uid == null || status == null) {
				throw new ActionException("id or status must be required");
			}
			...
			logger.warn("id不能为null, useid={}, status={}", uid, status, e);
		} catch (Exception e) {
	 ...
	 */
	private List<UserCustomer> getUserCustomers() throws ActionException {
		List<UserCustomer> userCustomers = new ArrayList<>();
		AuthUser myuser = getAuthUser();
		if(myuser == null) {
			throw new ActionException("session is timeout!");
		}
