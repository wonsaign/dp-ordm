			...
			<table id="table_position" class="table table-bordered table-striped">
				...
				<tr name="table_body" title="双击显示明细">
					...
					<td style="min-width:70px">
						<input class="lihref" type="hidden" value="../orderadm/init">
						<span href='../orderadm/orderPass' class="order_pass">通过</span>
						<span href='../orderadm/refuseOrder' class="order_refuse">退回</span>
					</td>
					<td id="${ob.orderNo}">${ob.orderNo}</td>
					<td>${ob.counterName}</td>
					<td>${ob.customerName}</td>
					<td <c:if test="${fn:contains(ob.usefulBalance,'-')}">style="color: red"</c:if>>${ob.usefulBalance}</td>
