...
<body>
	<div class="button_div">
		<div class="button_block">
			<input type="button" value="搜索" class="search_checked" href="../orderadm/searchInvalidOrder" />
			<input type="text" value="${name}" class="ip_text" />
		</div>
	</div>
	<div>
			...
			<tbody>
				<c:forEach items="${OrderBeanList}" var="ob" varStatus="s">
					<tr name="table_body" title="双击显示明细">
						...
						<td>
							<span href='../orderadm/orderDelete' class="order_delete" style="color: red">删除</span>
						</td>
						<td>${ob.orderNo}</td>
						...
						<td>
							<div class="img_rul" type="hidden">
								<c:forEach items="${ob.imageURL}" var="url" varStatus="url_s">
									<input type="hidden"
										value="<%=AppConfig.getVfsPrefix()%>${url}">
								</c:forEach>
							</div> <span <c:if test="${ob.img=='有'}">style="color: blue"</c:if>
							class="order_img_show">${ob.img}</span>
						</td>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-primary btn-primary-close"
	...
	<div class="pagination_div">
		<a hidden="hidden" name="paginationURL">../orderadm/invalidOrderPage</a>
