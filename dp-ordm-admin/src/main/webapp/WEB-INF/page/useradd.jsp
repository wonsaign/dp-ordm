...
<html>
	...
	<body>
		...
		<sf:form action="../useradm/createuser.do" method="post" enctype="multipart/form-data" >
		<div style="width:100%; background:#e6e6e6;" class="div_body">
			<div style="width:750px; color:#5bc0de; text-align:center; padding:10px 50px; font-weight:bold; margin:0px auto; height:30px; background:white; border:2px solid white; border-radius:5px;"> 用户创建</div>
			<div style="width:750px; margin:10px  auto  0px auto; padding:20px 50px; background:white; border:2px solid white; border-radius:5px;">
				<div style="width:650px; margin:20px auto; min-height:350px; height: auto;">
					...
					<ul class="user_text">
		    			...
		    			<li><span>用户名</span><input name="loginName" type="text" value="${authUser.loginName}"/></li>
		    			<li><span>密码</span><input name="password" type="password" value="${authUser.password}"/></li>
					...
					<div id="org_li">
    					...
    					<select class="user_select" name="orgUnit0">
				...
				<div class="user_role">
    				...
    				<input  name="Role_cascade" type="button" value="显示可选角色" href="../useradm/cascade">
	    			<ul name="roles_ul">
    					<c:if test="${not empty myRoles}">
			    			<c:forEach items="${allRoles}" var="allr" varStatus="vs">
			    				<input
			    					<c:forEach items="${myRoles}" var="myr"><c:if test="${myr.rid == allr.rid}">checked="checked"</c:if></c:forEach>  
			    					type="checkbox" name="rid" id="${allr.rid }" value="${allr.rid}"><label for="${allr.rid }">${allr.commonName}</label>&nbsp;
			    			</c:forEach>
			    		</c:if>
	    				<c:if test="${empty myRoles}">
			    			<c:forEach items="${allRoles}" var="allr" varStatus="vs">	    				
		    					<input type="checkbox" name="rid" id="${allr.rid }" value="${allr.rid}"><label for="${allr.rid }">${allr.commonName}</label>&nbsp;
			    			</c:forEach>
			    		</c:if>
					</ul>
    			</div>
				<div class="sure">
					<input type="button" value="创建" <c:if test="${createflag}">disabled="disabled"</c:if>  onclick="this.form.submit()" />
