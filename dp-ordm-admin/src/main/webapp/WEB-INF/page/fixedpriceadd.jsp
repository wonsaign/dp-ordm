...
<html>
	...
	<body>
		...
		<sf:form action="../fixedPrice/createFixedPrice.do" method="post" enctype="multipart/form-data" >
		<div style="width:100%; background:#e6e6e6;" class="div_body">
			...
			<div>
				...
				<div style="float:left; width:200px; position:relative;" class="gro_div">
					<input type="hidden" name="productId" class="pid">
					<input type="text" name="productName" class="prp_t mat productName" placeholder="请输入产品名称">
			...
			<span style="float:left;margin-right:20px;">一口价类型:</span>
				<select name="fix">
						<option value="1" selected="selected">固定价格</option>
						<option value="0">固定折扣</option>
				</select>
			</div>
			<div class="price_type">
			<span style="float:left;margin-right:20px;">产品折扣:</span>
			<input type="number" class="mat" name="discount" placeholder="请输入产品折扣">
			</div>
			<div class="price_type">
			<span style="float:left;margin-right:20px;">产品价格:</span>
			<input type="number" class="mat" name="price" placeholder="请输入产品价格">
			</div>
			<div>
			...
			<span style="float:left;margin-right:20px;">是否费比:</span>
				<select name="costRatio">
						<option value="1" selected="selected">走费比</option>
						<option value="0" >不走费比</option>
				</select>
			</div>
			<div class="clear"></div>
			<div>
				<div><span style="float:left;margin-right:20px;">客户类型:</span></div>
				<ul class="custype">
					<c:forEach items="${DSResponse.extra['customerType']}" var="cus">
						<li><label><input type="checkbox" name="customerTypeId" value="${cus.hardCode}" disabled>${cus.name}</label></li>
					</c:forEach>
					<div class="clear"></div>
				</ul>
				<div class="clear"></div>
				...
				<div class="sure">
					<input type="button" <c:if test="${DSResponse.extra['createflag']}">disabled="disabled"</c:if> name="createPRP" value="创建" />
