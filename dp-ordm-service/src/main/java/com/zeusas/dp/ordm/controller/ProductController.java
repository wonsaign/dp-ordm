...
public class ProductController extends OrdmBasicController {
	...
	public String getById(ModelMap mp,
		...
		try {
			...
			mp.addAttribute("isAvailableActivity", isAvailableActivity);
	...
	public String listMoreActi(ModelMap mp,
		...
		try {
			...
			mp.addAttribute("presents", presents);
			mp.addAttribute("materiels", materiels);
			// XXX:FIXME 活动升级，原接口过期处理？
			//List<ProductGroup> lpgs = productGroupManager.findAllGroup();
			//mp.addAttribute("activity", lpgs);
...
//	public byte[] findByBigType(
	...
	public String addptype(List<Product> products, String typeCode) {
		// TODO 給List<Proudct>添加自定义的类型
		return null;
	}

	@RequestMapping("deleteproducttype")
	public String deletetype(List<Product> products, String typeCode) {
		// TODO 給List<Proudct>删除自定义的类型
		return null;
	}
...
//	}
	...
	protected void addDefaultQty(List<Product> products) {
		String counterCode;
		if (super.getAllCounters().size() > 1) {
			if (super.getCounterCode() == null) {
				counterCode = super.getDefaultCounter();
			} else {
				counterCode = super.getCounterCode();
			}
		} else {
			counterCode = super.getCounterCode();
		}
	...
	private void changeActFlag(List<Product> pp) {
		UserActivities ua = super.getUserActivityList();
		pp.forEach(p -> {
	 ...
	 * @return
	 */
	private List<Product> wrapProduct(List<Product> products,ModelMap mp) {
		// 设置“活动商品”上图标
		changeActFlag(products);
		// 添加默认的销量
		addDefaultQty(products);
	 ...
	 * @return
	 */
	private List<Product> wrapProduct(List<Product> products) {
		// 过滤预定会限制的产品
		products = filterRevProduct(super.getCounterCode(), products);
		// 设置“活动商品”上图标
		changeActFlag(products);
